...
const CreateAccount = ({ handleNext }) => {
  ...
  const validateForm = () => {
    ...
    if (!liquorLicense) newErrors.liquorLicense = "Liquor license copy is required.";
    if (uploadedImages.length === 0) newErrors.uploadedImages = "FSSAI license copy is required.";
    ...
    <div>
      <div className="new-verify-form">
        <form className="login-form" onSubmit={handleFormSubmit}>
          ...
          <label htmlFor="panno" className="login-label">PAN Card Number</label>
          <input
            type="text"
            id="panno"
            className="login-input"
            value={panno}
            onChange={handleChange(setPanno, "panno", /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/)}
            required
          />
          ...
          <div className="file-upload-container">
            ...
            <div className="preview-container">
              {uploadedImages.map((image, index) => (
                <div key={index} className="uploaded-image-wrapper">
                  <img src={image} alt="FSSAI License" className="uploaded-image" />
                  <FaTimes
            ...
            {errors.uploadedImages && <span className="error">{errors.uploadedImages}</span>}
          </div>

          {/* Liquor License Section */}
          <label htmlFor="liquor-license-upload" className="login-label">Liquor license copy</label>
          <div className="file-upload-container">
            <input
              type="file"
              id="liquor-license-upload"
              className="file-input"
              onChange={(e) => handleFileUpload(e.target.files, setLiquorLicense)}
              required
            />
            <label htmlFor="liquor-license-upload" className="file-upload-label">
              <div className="upload-content">
                <IoCloudUploadOutline className="upload-icon" />
                <p>Browse and choose the file you want to upload from your computer</p>
                <AiFillPlusSquare className="upload-btn" />
              </div>
            </label>
            {liquorLicense && (
              <div className="uploaded-image-container">
                <img src={liquorLicense} alt="Liquor License" className="uploaded-image" />
              </div>
            )}
            {errors.liquorLicense && <span className="error">{errors.liquorLicense}</span>}
          </div>
...
//           formData.append(`fssai_license_${index + 1}`, image);
//         });
